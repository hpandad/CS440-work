...
class q_learner():
    def __init__(self, alpha, epsilon, gamma, nfirst, state_cardinality):
        ...
        a = float
        b = int
        self.Q = np.zeros((tuple(state_cardinality + [3])), dtype=a)
        self.N = np.zeros((tuple(state_cardinality + [3])), dtype=b)
    ...
    def choose_unexplored_action(self, state):
        ...
        '''
        explored = self.report_exploration_counts(state)
        c = 3
        n = self.nfirst
        unexplored = []
        for i in range(3):
    ...
    def learn(self, state, action, reward, newstate):
        ...
        None
        '''
        temp = tuple(state + [action + 1])
        temp2 = self.Q[temp]
        a = self.alpha
        self.Q[temp] =  temp2 + a * (self.q_local(reward, newstate) - temp2)
        # raise RuntimeError('You need to write this!')
    ...
    def exploit(self, state):
        ...
        '''
        return ((np.argmax(self.report_q(state))-1), np.max(self.report_q(state)[np.argmax(self.report_q(state))]))
    ...
    def act(self, state):
        ...
        '''
        temp = self.choose_unexplored_action(state)
        if temp == None:
            e = self.epsilon
            if (random.random() <= e):
